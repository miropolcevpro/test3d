...
// --- StoneMix palette scrollbar/progress indicator (landing page only) ---
function initStonemixPaletteScrollbar(){
  const scroller = document.querySelector('.stonemixPaletteScroll');
  const bar = document.querySelector('.stonemixPaletteScrollbar');
  const thumb = document.querySelector('.stonemixPaletteScrollbarThumb');
  ...
  const update = () => {
    ...
    const thumbW = Math.max(0.18, Math.min(0.65, ratio)) * 100;
    thumb.style.width = thumbW + '%';
    const pos = (scroller.scrollLeft / max) * (100 - thumbW);
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initStonemixPaletteScrollbar, { once: true });
} else {
  initStonemixPaletteScrollbar();
}
