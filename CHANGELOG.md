## 20260422-f24dy
- Leads: добавлен безопасный backend relay `POST /api/telemetry?mode=lead_submit`, который принимает заявки из калькулятора и формы связи и отправляет их в Telegram.
- Calculator: standalone transport теперь по умолчанию отправляет заявки в backend endpoint без Tilda-зависимости и без bot token во фронтенде.
- Contact form: кнопка «Связь с менеджером» открывает аккуратную форму заявки; в Telegram уходят контакты, комментарий, контекст текущей формы/текстуры и служебные метаданные.
- Release token: `20260422-f24dy`
